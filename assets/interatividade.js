...
(function () {
  ...
  // Escolhe um passo "redondo" (1/2/5 x uma potencia de 10 - ex.: 50,
  // 100, 500, 1000...) pro eixo de preco, ao inves de dividir min/max em
  // fatias iguais (que dao numeros quebrados tipo "R$ 433,25"). Quanto
  // maior o intervalo entre o menor e o maior preco, maior o passo
  // escolhido - e assim que graficos financeiros de verdade rotulam um
  // eixo de valores.
  function _ticksNiceis(minimoDados, maximoDados, alvoTicks) {
    var faixa = maximoDados - minimoDados;
    if (faixa <= 0) faixa = Math.max(1, Math.abs(maximoDados) || 1);
    var passoBruto = faixa / Math.max(1, alvoTicks - 1);
    var magnitude = Math.pow(10, Math.floor(Math.log(passoBruto) / Math.LN10));
    var residuo = passoBruto / magnitude;
    var passo = residuo > 5 ? 10 * magnitude : residuo > 2 ? 5 * magnitude : residuo > 1 ? 2 * magnitude : magnitude;
    var minimo = Math.floor(minimoDados / passo) * passo;
    var maximo = Math.ceil(maximoDados / passo) * passo;
    var ticks = [];
    for (var v = minimo; v <= maximo + passo * 0.001; v += passo) {
      ticks.push(Math.round(v * 100) / 100);
    }
    return { minimo: minimo, maximo: maximo, passo: passo, ticks: ticks };
  }
  ...
  function renderizarComparativo(painel) {
    ...
    var topo = 30, altura = 300, esquerda = 110, largura = 1000 - esquerda - 30;
    var passo = largura / (n - 1);

    // UM UNICO eixo de preco real (R$), compartilhado por todas as
    // commodities selecionadas - a posicao no grafico sempre corresponde
    // ao valor de fato: uma commodity que vale mais aparece SEMPRE mais
    // alta que uma que vale menos, em qualquer trecho do periodo, sem
    // excecao (um eixo por commodity permitia o oposto - a mais cara
    // parecendo "mais baixa" em algum ponto so por estar perto do
    // proprio minimo dela, o que confunde mais do que ajuda). As marcas
    // do eixo usam numeros redondos (ver _ticksNiceis) - o passo entre
    // elas cresce sozinho quando o intervalo de preco e maior.
    var todosPrecos = [].concat.apply([], series.map(function (item) { return item.valores.map(function (p) { return p[1]; }); }));
    var minimoDados = Math.min.apply(null, todosPrecos), maximoDados = Math.max.apply(null, todosPrecos);
    if (minimoDados === maximoDados) { minimoDados -= 1; maximoDados += 1; }
    var ticksInfo = _ticksNiceis(minimoDados, maximoDados, 6);
    var minimo = ticksInfo.minimo, faixa = ticksInfo.maximo - ticksInfo.minimo;

    function posicaoY(_item, valor) {
      return topo + altura - ((valor - minimo) / faixa) * altura;
    }

    svg.setAttribute("viewBox", "0 0 1000 420"); svg.innerHTML = "";

    ticksInfo.ticks.forEach(function (valorTick, indiceTick) {
      ...
      if (indiceTick > 0) {
        ...
        linhaGrade.setAttribute("y1", yTick); linhaGrade.setAttribute("y2", yTick);
    ...
    document.head.appendChild(estiloImpressao);
      document.querySelectorAll(".chart-ferramentas").forEach(function (ferramentas) {
        ...
        if (dados && base && grafico && !document.querySelector(".comparativo-painel")) {
          ...
          painel.innerHTML = '<div class="comparativo-cabecalho"><div><span class="section-label">Comparação</span><h3>Evolução comparada</h3><p>Preço real (não variação percentual) de todas as commodities selecionadas, no mesmo eixo - quem vale mais aparece sempre mais alto no gráfico. Passe o mouse para ver o valor de cada dia.</p></div><div class="comparativo-acoes"><button type="button" class="chart-btn-secundario comparativo-exportar">Exportar em PDF</button><button type="button" class="comparativo-limpar">Limpar seleção</button></div></div><div class="comparativo-controles"><fieldset><legend>Commodities</legend><div class="comparativo-opcoes"></div></fieldset><label>Série<select class="comparativo-serie"><option value="fisicos">Preço físico</option><option value="futuros">Preço futuro</option></select></label><label>Período<select class="comparativo-periodo"><option value="7">7 dias</option><option value="30">30 dias</option><option value="90" selected>90 dias</option><option value="tudo">Tudo</option></select></label></div><div class="comparativo-legenda"></div><p class="comparativo-leitura">Passe o mouse sobre o gráfico para ver o preço de cada dia.</p><svg class="comparativo-svg" role="img" aria-label="Comparação de preço real entre commodities, na mesma escala"></svg><p class="comparativo-vazio">Selecione uma ou mais commodities para iniciar a comparação.</p>';
